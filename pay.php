...
<head>
  ...
  <script>
    ...
    // SECURE+: Hiển thị thông tin ngân hàng sau khi tải trang
    window.addEventListener('DOMContentLoaded', function() {
      var bankData = decodeBankInfo('<?= $secureData ?>');
      if (bankData) {
        ...
        document.getElementById('bank-holder').textContent = bankData.account_holder || '';
        if (bankData.branch) {
          document.getElementById('bank-branch-row').style.display = '';
          document.getElementById('bank-branch').textContent = bankData.branch;
        }
      }
    });
  ...
  <style>
    ...
    /* SECURE+: Ẩn thông tin trước khi JavaScript load */
    .secure-info { 
      filter: blur(8px); 
      user-select: none;
...
<div class="container mt-5">
  ...
  <div class="card shadow-sm">
    ...
    <div class="card-body p-4">
      ...
      <?php if (!empty($bank)): ?>
        <!-- SECURE+: Thông tin ngân hàng được mã hóa và chỉ hiển thị qua JavaScript -->
        <ul class="list-unstyled">
          ...
          <li id="bank-branch-row" style="display:none;"><strong>Chi nhánh:</strong> <span id="bank-branch" class="secure-info">***</span></li>
          <li><strong>Nội dung chuyển khoản:</strong> <span class="fw-bold text-danger"><?= h($tx['unique_code']) ?></span></li>
        </ul>
        
        <script>
          // SECURE+: Xóa blur sau khi load xong
          window.addEventListener('DOMContentLoaded', function() {
            setTimeout(function() {
              document.querySelectorAll('.secure-info').forEach(function(el) {
