...
# --- CÁC MENU KHÁC (Tương tự code single-server nhưng dùng selected server) ---
            ...
            if st.button("✨ Tạo Ngay", type="primary", use_container_width=True):
                if not remark:
                    ...
                    if res:
                        ...
                        with col_qr:
                            st.image(generate_qr(link), width=200)
                    else:
                        st.error(f"❌ Lỗi: {msg}")
    ...
    # --- CHI TIẾT USER ---
        ...
        if inbounds:
            ...
            if user_data:
                ...
                with col_qr:
                    st.image(generate_qr(link), width=250)
        else:
...
st.write("---")
st.caption("© 2024 VPN Admin Pro - Multi-Server Edition")
