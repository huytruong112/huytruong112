...
def update_inbound(session, host, inbound_id, inbounds, new_remark=None, new_days=None, new_data_limit_gb=None):
    """Cập nhật thông tin inbound - WORKAROUND"""
    try:
        st.warning("⚠️ **Chức năng Edit User hiện không hoạt động do API issue**")
        st.info("""
        **WORKAROUND:**
        1. Xóa user này (button 🗑️ - ✅ hoạt động)
        2. Tạo user mới với thông tin mới (✅ hoạt động)
        
        Hoặc edit trực tiếp trên 3X-UI panel gốc.
        """)
        return False, "API update không hoạt động - Vui lòng xóa và tạo lại"
    except Exception as e:
        return False, str(e)

def reset_traffic(session, host, inbound_id, inbounds):
    """Reset traffic của inbound - TRY SPECIALIZED ENDPOINT"""
    try:
        # Try specialized reset endpoint
        resp = session.post(
            f"{host}/xui/inbound/resetAllTraffics",
            json={"id": inbound_id},
            timeout=10
        )
        ...
        resp = session.post(
            f"{host}/xui/inbound/resetClientTraffic/{inbound_id}",
            timeout=10
        )
        ...
        st.warning("⚠️ Reset traffic không hoạt động qua API")
        st.info("ℹ️ Vui lòng reset trực tiếp trên 3X-UI panel")
        return False
...
def extend_expiry(session, host, inbound_id, additional_days, inbounds):
    """Gia hạn thêm ngày - WORKAROUND FOR API LIMITATION"""
    try:
        st.warning("⚠️ **Chức năng Gia hạn hiện không hoạt động do 3X-UI API limitation (HTTP 500)**")
        st.info("""
        **WORKAROUND - Chọn 1 trong 2 cách:**
        
        **Cách 1: Xóa & Tạo lại** (Khuyến nghị)
        1. Note lại thông tin client (remark, protocol, settings)
        2. Click 🗑️ Xóa client này (✅ hoạt động)
        3. Click ➕ Tạo Mới với thông tin giống và expiry mới (✅ hoạt động)
        
        **Cách 2: Sửa trực tiếp trên 3X-UI Panel**
        1. Mở 3X-UI panel gốc
        2. Vào Inbounds → Edit inbound này  
        3. Thay đổi Expiry Time
        4. Click Update
        
        **Note:** Tôi đang làm việc để fix API này trong phiên bản tiếp theo!
        """)
        return False
    except Exception as e:
...
def toggle_inbound(session, host, inbound_id, enable, inbounds):
    """Bật/tắt inbound - TRY SIMPLE METHOD"""
    try:
        ...
        with st.expander("🔍 Debug - Toggle", expanded=False):
            st.write("**Trying simple method:**")
            st.json(simple_payload)
        ...
        st.warning(f"⚠️ Toggle không hoạt động: HTTP {resp.status_code}")
        st.info("ℹ️ Vui lòng toggle trực tiếp trên 3X-UI panel")
        return False
...
st.write("---")
st.caption("© 2024 VPN Admin Pro - Multi-Server Edition v2.3.4 - API Limitations Workaround")
